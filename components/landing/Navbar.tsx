...
export function Navbar() {
  ...
  return (
    ...
    >
      <nav
        className={`relative mx-auto flex max-w-6xl items-center justify-between rounded-2xl border px-4 py-3 transition-all duration-300 sm:px-5 ${
          scrolled
            ? "border-white/10 bg-[#0b0d1f]/80 shadow-2xl shadow-black/40 backdrop-blur-2xl"
            : "border-white/10 bg-white/[0.04] backdrop-blur-xl"
        }`}
      >
        <div className="pointer-events-none absolute -top-20 left-1/2 h-32 w-3/4 -translate-x-1/2 rounded-full bg-violet-600/20 blur-3xl" />
        ...
        <div className="relative hidden items-center gap-2 md:flex">
          ...
          <MagneticButton
            href="/signup"
            className="group relative overflow-hidden rounded-xl bg-gradient-to-r from-amber-400 to-orange-600 px-5 py-2 text-sm font-semibold text-slate-950 shadow-lg shadow-orange-500/30"
          >
        ...
        <button
          onClick={() => setOpen((v) => !v)}
          className="relative grid h-10 w-10 place-items-center rounded-xl glass md:hidden"
      ...
      {open && (
        <div className="mx-auto mt-2 max-w-6xl rounded-2xl border border-white/10 bg-[#0b0d1f]/90 p-4 shadow-2xl shadow-black/40 backdrop-blur-2xl md:hidden">
